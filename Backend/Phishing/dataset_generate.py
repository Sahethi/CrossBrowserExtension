...
def generate_data_set(url):
    ...
    try:
        rank = BeautifulSoup(
            requests.get("http://data.alexa.com/data?cli=10&dat=s&url=" + url).read(),
            "xml",
        ).find("REACH")["RANK"]
        rank = int(rank)
        if rank < 100000:
...
def predict(url):
    ...
    model = pickle.load(
        open(
            "/Users/ayushpattnaik/Documents/Developer/CrossBrowserExtension/Backend/Phishing/my_model.pickle",
            "rb",
        )
    )
